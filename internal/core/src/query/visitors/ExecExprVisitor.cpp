...
ExecExprVisitor::ExecBinaryArithOpEvalRangeVisitorDispatcher(BinaryArithOpEvalRangeExpr& expr_raw) -> BitsetType {
    // see also: https://github.com/milvus-io/milvus/issues/23646.
    typedef std::conditional_t<std::is_integral_v<T> && !std::is_same_v<bool, T>, int64_t, T> HighPrecisionType;
...
ExecExprVisitor::ExecBinaryRangeVisitorDispatcher(BinaryRangeExpr& expr_raw) -> BitsetType {
    ...
    // see also: https://github.com/milvus-io/milvus/issues/23646.
    typedef std::conditional_t<std::is_integral_v<IndexInnerType> && !std::is_same_v<bool, T>, int64_t, IndexInnerType>
        HighPrecisionType;
    ...
    if (use_pk_index) {
        ...
        switch (field_meta.get_data_type()) {
            ...
            case DataType::VARCHAR: {
                ...
                for (const auto& id : expr.terms_) {
                    std::cout << "dyh add_data 1" << std::endl;
                    dst_ids->add_data((std::string&)id);
                    std::cout << "dyh add_data 2" << std::endl;
                }
                break;
            }
            default: {
        ...
        std::cout << "dyh search ids" << std::endl;
