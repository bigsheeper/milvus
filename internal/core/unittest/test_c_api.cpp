...
TEST(CApiTest, ReduceSearchWithExprV2) {
    ...
    // 2. marshal
    CSearchResultData cSearchResultData = nullptr;
    status = Marshal(&cSearchResultData, results.data(), results.size(), nullptr, -1);
    assert(status.error_code == Success);

    auto search_result = static_cast<milvus::proto::schema::SearchResultData*>(cSearchResultData);
