...
func (h *ServerHandler) GetDataVChanPositions(channel *channel, partitionID UniqueID) *datapb.VchannelInfo {
	...
	return &datapb.VchannelInfo{
		...
		SeekPosition:        h.getVChanSeekPosition(channel),
...
func (h *ServerHandler) getVChanSeekPosition(channel *channel) *internalpb.MsgPosition {
	seekPosition := h.s.meta.GetChannelPosition(channel.Name)
	if seekPosition != nil {
		log.Info("channel seek position set from ChannelPosition",
			zap.Uint64("position timestamp", seekPosition.Timestamp),
			zap.String("realworld position timestamp", tsoutil.ParseAndFormatHybridTs(seekPosition.Timestamp)),
		)
	} else {
