...
func (s *SegmentManager) CleanZeroSealedSegmentsOfChannel(channel string, cpTs Timestamp) {
	...
	sealed, ok := s.channel2Sealed.Get(channel)
	if !ok {
		log.Info("czs try remove empty sealed segment after channel cp updated failed to get channel", zap.String("channel", channel))
	...
	sealed.Range(func(id int64) bool {
		segment := s.meta.GetHealthySegment(id)
		if segment == nil {
			log.Info("czs try remove empty sealed segment after channel cp updated failed to get segment")
		...
		log.Info("czs try remove empty sealed segment after channel cp updated",
			zap.Int64("collection", segment.CollectionID), zap.Int64("segment", id),
			zap.String("channel", channel), zap.Any("cpTs", cpTs), zap.Int64("currentRows", segment.NumOfRows),
			zap.Any("expireTs", segment.GetLastExpireTime()))
		if segment.GetLastExpireTime() <= cpTs && segment.currRows == 0 {
