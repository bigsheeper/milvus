...
type Channel interface {
	...
	setChannelCheckpoint(ttPos *internalpb.MsgPosition)
...
func (c *ChannelMeta) addSegment(req addSegmentReq) error {
	...
	seg := &Segment{
		...
		curInsertBuf:     insertBuf,
		curDeleteBuf:     deleteBuf,
...
func (c *ChannelMeta) setChannelCheckpoint(ttPos *internalpb.MsgPosition) {
	c.segMu.RLock()
	defer c.segMu.RUnlock()
	...
	for _, seg := range c.segments {
		if seg.curInsertBuf.startPos != nil && seg.curInsertBuf.startPos.Timestamp < channelCP.Timestamp {
			channelCP = seg.curInsertBuf.startPos
		}
		if seg.curDeleteBuf.startPos != nil && seg.curDeleteBuf.startPos.Timestamp < channelCP.Timestamp {
			channelCP = seg.curDeleteBuf.startPos
		}
		for _, ib := range seg.historyInsertBuf {
			if ib != nil && ib.startPos.Timestamp < channelCP.Timestamp {
				channelCP = ib.startPos
			}
		}
		for _, db := range seg.historyDeleteBuf {
			if db != nil && db.startPos.Timestamp < channelCP.Timestamp {
...
func (c *ChannelMeta) rollInsertBuffer(segmentID UniqueID) error {
	...
	seg, ok := c.segments[segmentID]
	if ok {
		collSchema, err := c.getCollectionSchema(c.collectionID, 0)
		if err != nil {
			log.Error("Get schema wrong:", zap.Error(err))
			return err
		}
		return seg.rollInsertBuffer(collSchema)
	}
