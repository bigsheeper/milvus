...
func newDmInputNode(ctx context.Context, seekPos *internalpb.MsgPosition, dmNodeConfig *nodeConfig) (*flowgraph.InputNode, error) {
	...
	if seekPos != nil && len(seekPos.MsgID) != 0 {
		input, err = dispatcherManager.Register(dmNodeConfig.vChannelName, seekPos, mqwrapper.SubscriptionPositionUnknown)
		if err != nil {
			return nil, err
		}
		...
		input, err = dispatcherManager.Register(dmNodeConfig.vChannelName, nil, mqwrapper.SubscriptionPositionEarliest)
		if err != nil {
			return nil, err
		}
