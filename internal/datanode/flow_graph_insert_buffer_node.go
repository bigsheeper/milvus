...
func (ibNode *insertBufferNode) Operate(in []Msg) []Msg {
	...
	if startPositions[0].Timestamp < ibNode.lastTimestamp {
		...
		panic(fmt.Errorf("insert buffer node consumed old messages, channel = %s, timestamp = %d, lastTimestamp = %d",
	...
	if err != nil {
		...
		panic(fmt.Errorf("update segment states in Replica wrong, err = %s", err))
	}
	...
	for _, msg := range fgMsg.insertMessages {
		...
		if err != nil {
			...
			panic(fmt.Errorf("insertBufferNode msg to buffer failed, err = %s", err))
		}
	}
	...
	for _, task := range flushTaskList {
		...
		if err != nil {
			...
			panic(fmt.Errorf("insertBufferNode flushBufferData failed, err = %s", err))
		}
