...
func (ttn *ttNode) Operate(in []Msg) []Msg {
	...
	ttn.channel.setChannelCheckPoint(fgMsg.endPositions[0])

	curTs, _ := tsoutil.ParseTS(fgMsg.timeRange.timestampMax)
	if curTs.Sub(ttn.lastUpdateTs) >= updateChanCPInterval {
		ttn.updateChannelCheckPoint()
		ttn.lastUpdateTs = curTs
	}

	return []Msg{}
}

func (ttn *ttNode) updateChannelCheckPoint() {
	channelPos := ttn.channel.getChannelCheckPoint()
	if channelPos == nil || channelPos.MsgID == nil {
		log.Warn("updateChannelCheckPoint failed, get nil check point", zap.String("vChannel", ttn.vChannelName))
		return
	}
	...
	defer cancel()
	resp, err := ttn.dataCoord.UpdateChannelPosition(ctx, &datapb.UpdateChannelPositionRequest{
	...
	if err = funcutil.VerifyResponse(resp, err); err != nil {
		log.Warn("UpdateChannelPosition failed", zap.String("channel", ttn.vChannelName),
...
func newTTNode(config *nodeConfig, dc types.DataCoord) (*ttNode, error) {
	...
	tt := &ttNode{
		...
		lastUpdateTs: time.Time{}, // set to Zero to update channel checkPoint immediately when fg started
		dataCoord:    dc,
	}
