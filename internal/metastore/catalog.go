...
type DataCoordCatalog interface {
	...
	ListChannelPositions(ctx context.Context) (map[string]*internalpb.MsgPosition, error)
	SaveChannelPosition(ctx context.Context, vChannel string, pos *internalpb.MsgPosition) error
	RemoveChannelPosition(ctx context.Context, vChannel string) error
}
