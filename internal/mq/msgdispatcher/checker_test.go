...
func TestChecker(t *testing.T) {
	t.Run("test add and remove dispatcher", func(t *testing.T) {
		c := newChecker("mock_pchannel_0", typeutil.ProxyRole, 1, newMockFactory())
		assert.NotNil(t, c)
...
func (suite *SimulationSuite) SetupTest() {
	suite.pchannel = fmt.Sprintf("by-dev-rootcoord-dispatcher-simulation-dml-%d", rand.Int())
	...
	for i := 1; i <= timeTickCount; i++ {
		...
		for j := 0; j < insNum; j++ {
			...
			err := suite.producer.Produce(&msgstream.MsgPack{
				Msgs: []msgstream.TsMsg{genInsertMsg(rand.Intn(200)+1, vchannel, uniqueMsgID)},
			})
			assert.NoError(suite.T(), err)
			uniqueMsgID++
...
func (suite *SimulationSuite) TestMerge() {
	...
	for i := 0; i < vchannelNum; i++ {
		vchannel := fmt.Sprintf("%s_vchannelv%d", suite.pchannel, i)
		output, err := suite.checker.addDispatcher(vchannel, positions[rand.Intn(len(positions))],
