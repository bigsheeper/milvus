...
var (
	...
	// either because it was never rotated or because it was already dropped.
...
func (inc *incrementDB) destroy() error {
	if err := inc.db.Close(); err != nil {
		return sst.MarkPebbleErr(errors.Wrapf(err, "close increment %s", inc.dir))
	}
	if err := os.RemoveAll(inc.dir); err != nil {
		return pkerr.MarkIO(err, "remove increment %s", inc.dir)
	}
	return nil
}
...
func (e *pebbleEngine) openIncrement(gen Generation) (*incrementDB, error) {
	...
	if err != nil {
		return nil, sst.MarkPebbleErr(errors.Wrapf(err, "open increment %s", dir))
	}
...
func (e *pebbleEngine) Apply(ctx context.Context, muts []Mutation) error {
	...
	for _, m := range muts {
		...
		if err := b.Set(m.Key, v, nil); err != nil {
			return sst.MarkPebbleErr(errors.Wrapf(err, "stage write to increment %d", e.active.gen))
		}
	}
	if err := b.Commit(pebble.NoSync); err != nil {
		return sst.MarkPebbleErr(errors.Wrapf(err, "commit write to increment %d", e.active.gen))
	}
	return nil
}
...
func (e *pebbleEngine) probeIncrement(inc *incrementDB, key []byte) (value []byte, found bool, err error) {
	...
	if err != nil {
		return nil, false, sst.MarkPebbleErr(errors.Wrapf(err, "read increment %s", inc.dir))
	}
	defer closer.Close()
...
func (e *pebbleEngine) FlushDraining(ctx context.Context, gen Generation) ([]sst.Info, error) {
	...
	if err := inc.db.Flush(); err != nil {
		return nil, sst.MarkPebbleErr(errors.Wrapf(err, "flush increment %s", inc.dir))
	}
	levels, err := inc.db.SSTables()
	if err != nil {
		return nil, sst.MarkPebbleErr(errors.Wrapf(err, "list ssts of increment %s", inc.dir))
	}
...
func (e *pebbleEngine) PutLocalMeta(key string, val []byte) error {
	...
	if err != nil {
		return pkerr.MarkIO(err, "create temp meta in %s", dir)
	}
	if _, err := tmp.Write(val); err != nil {
		tmp.Close()
		os.Remove(tmp.Name())
		return pkerr.MarkIO(err, "write meta %s", tmp.Name())
	}
	if err := tmp.Sync(); err != nil {
		tmp.Close()
		os.Remove(tmp.Name())
		return pkerr.MarkIO(err, "sync meta %s", tmp.Name())
	}
	tmp.Close()
	if err := os.Rename(tmp.Name(), filepath.Join(dir, key)); err != nil {
		os.Remove(tmp.Name())
		return pkerr.MarkIO(err, "publish meta %s in %s", key, dir)
	}
	return nil
}
...
func (e *pebbleEngine) GetLocalMeta(key string) ([]byte, bool, error) {
	...
	if err != nil {
		return nil, false, pkerr.MarkIO(err, "read meta %s", key)
	}
	return b, true, nil
}
