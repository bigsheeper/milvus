...
func TestRateLimitInterceptor(t *testing.T) {
	...
	t.Run("test getFailedResponse", func(t *testing.T) {
		testGetFailedResponse := func(req interface{}, rt internalpb.RateType, err error, fullMethod string) {
			rsp := getFailedResponse(req, err, fullMethod)
			assert.NotNil(t, rsp)
		}
		...
		// test illegal
		rsp := getFailedResponse(&milvuspb.SearchResults{}, merr.OldCodeToMerr(commonpb.ErrorCode_UnexpectedError), "method")
		assert.Nil(t, rsp)
		rsp = getFailedResponse(nil, merr.OldCodeToMerr(commonpb.ErrorCode_UnexpectedError), "method")
		assert.Nil(t, rsp)
	})
