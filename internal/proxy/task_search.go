...
func doRequery(ctx context.Context,
	...
	for i := 0; i < typeutil.GetSizeOfIDs(ids); i++ {
		...
		if _, ok := offsets[id]; !ok {
			err = merr.WrapErrInconsistentRequery(fmt.Sprintf("incomplete query result, missing id %s, len(searchIDs) = %d, len(queryIDs) = %d, collection=%d",
				id, typeutil.GetSizeOfIDs(ids), len(offsets), collectionID))
			log.Ctx(ctx).Warn("dyh debug", zap.Error(err))
			return err
		}
