...
func (ut *upsertTaskByStreamingService) Execute(ctx context.Context) error {
	...
	if err := streaming.WAL().Append(ctx, deleteMsgs...).UnwrapFirstError(); err != nil {
		log.Warn("append delete message to wal failed", zap.Error(err))
		return err
	}

	if err := streaming.WAL().Append(ctx, insertMsgs...).UnwrapFirstError(); err != nil {
		log.Warn("append insert message to wal failed", zap.Error(err))
		return err
	}
	return nil
}
