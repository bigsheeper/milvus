...
func (c *LeaderChecker) findNeedLoadedSegments(ctx context.Context, replica *meta.Replica, leaderView *meta.LeaderView, dist []*meta.Segment) []task.Task {
	...
	for _, s := range latestNodeDist {
		...
		if !ok || version.GetNodeID() != s.Node {
			...
			// segment version is not need by delegator any more, just use the latest one  to be compatible with old version
			latestSegmentVersion := s.Version
			action := task.NewLeaderAction(leaderView.ID, s.Node, task.ActionTypeGrow, s.GetInsertChannel(), s.GetID(), latestSegmentVersion)
