...
func benchmarkQueryCollectionSearch(nq int64, b *testing.B) {
	...
	err = queryShardObj.streaming.replica.removeSegment(defaultSegmentID)
	assert.NoError(b, err)
	err = queryShardObj.historical.replica.removeSegment(defaultSegmentID)
	assert.NoError(b, err)

	assert.Equal(b, 0, queryShardObj.historical.replica.getSegmentNum())
	assert.Equal(b, 0, queryShardObj.streaming.replica.getSegmentNum())

	segment, err := genSimpleSealedSegment(nb)
	assert.NoError(b, err)
	err = queryShardObj.historical.replica.setSegment(segment)
	assert.NoError(b, err)

	// segment check
	assert.Equal(b, 1, queryShardObj.historical.replica.getSegmentNum())
	assert.Equal(b, 0, queryShardObj.streaming.replica.getSegmentNum())
	seg, err := queryShardObj.historical.replica.getSegmentByID(defaultSegmentID)
	assert.NoError(b, err)
	...
	collection, err := queryShardObj.historical.replica.getCollectionByID(defaultCollectionID)
	assert.NoError(b, err)
	...
	assert.NoError(b, err)
	for j := 0; j < 10000; j++ {
		_, _, err = queryShardObj.historical.searchSegments(searchReq, queryReq.GetSegmentIDs())
		assert.NoError(b, err)
	}
...
func benchmarkQueryCollectionSearchIndex(nq int64, indexType string, b *testing.B) {
	...
	assert.NoError(b, err)
	node.loader.historicalReplica = queryShardObj.historical.replica
	...
	searchReq, err := genSearchPlanAndRequests(collection, indexType, nq)
	for j := 0; j < 10000; j++ {
		_, _, err = queryShardObj.historical.searchSegments(searchReq, []UniqueID{defaultSegmentID})
		assert.NoError(b, err)
	}
	...
	for i := 0; i < b.N; i++ {
		for j := 0; j < benchmarkMaxNQ/int(nq); j++ {
			_, _, err = queryShardObj.historical.searchSegments(reqs[j], []UniqueID{defaultSegmentID})
			assert.NoError(b, err)
		}
	}
}
