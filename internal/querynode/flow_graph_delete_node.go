...
func (dNode *deleteNode) Operate(in []flowgraph.Msg) []flowgraph.Msg {
	...
	for i, delMsg := range dMsg.deleteMessages {
		...
		if dNode.replica.getSegmentNum() != 0 {
			...
			if err != nil {
				...
				panic(fmt.Errorf("deleteNode processDeleteMessages failed, collectionID = %d, err = %s", delMsg.CollectionID, err))
			}
		}
	}
	...
	for segmentID, pks := range delData.deleteIDs {
		...
		if err != nil {
			...
			panic(fmt.Errorf("deleteNode getSegmentByID failed, err = %s", err))
		}
