...
func (iNode *insertNode) Operate(in []flowgraph.Msg) []flowgraph.Msg {
	...
	for _, insertMsg := range iMsg.insertMessages {
		...
		if err != nil {
			...
			panic(fmt.Errorf("insertNode getCollectionByID failed, err = %s", err))
		}
		if col.getLoadType() == loadTypeCollection {
			...
			if err != nil {
				...
				panic(fmt.Errorf("insertNode addPartition failed, err = %s", err))
			}
		}
		...
		if !iNode.streamingReplica.hasSegment(insertMsg.SegmentID) {
			...
			if err != nil {
				...
				panic(fmt.Errorf("insertNode addSegment failed, err = %s", err))
			}
		}
		...
		if err != nil {
			...
			panic(fmt.Errorf("failed to transfer msgStream.insertMsg to storage.InsertRecord, err = %s", err))
		}
		...
		if err != nil {
			...
			panic(fmt.Errorf("failed to get primary keys, err = %d", err))
		}
	...
	for segmentID := range iData.insertRecords {
		...
		if err != nil {
			...
			panic(fmt.Errorf("insertNode getSegmentByID failed, err = %s", err))
		}
		...
		if targetSegment != nil {
			...
			if err != nil {
				...
				panic(fmt.Errorf("segmentPreInsert failed, segmentID = %d, err = %s", segmentID, err))
			}
	...
	for segmentID := range iData.insertRecords {
		...
		go func() {
			...
			if err != nil {
				...
				panic(fmt.Errorf("segment insert failed, segmentID = %d, err = %s", segmentID, err))
			}
		}()
	}
	wg.Wait()
	...
	for _, delMsg := range iMsg.deleteMessages {
		if iNode.streamingReplica.getSegmentNum() != 0 {
			...
			if err != nil {
				...
				panic(fmt.Errorf("insertNode processDeleteMessages failed, collectionID = %d, err = %s", delMsg.CollectionID, err))
			}
		}
	}
	...
	for segmentID := range delData.deleteOffset {
		...
		go func() {
			...
			if err != nil {
				...
				panic(fmt.Errorf("segment delete failed, segmentID = %d, err = %s", segmentID, err))
			}
		}()
	}
	wg.Wait()
