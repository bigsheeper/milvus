...
func (iNode *insertNode) Operate(in []flowgraph.Msg) []flowgraph.Msg {
	...
	for _, delMsg := range iMsg.deleteMessages {
		if iNode.metaReplica.getSegmentNum(segmentTypeGrowing) != 0 {
			...
			err := processDeleteMessages(iNode.metaReplica, delMsg, delData)
			if err != nil {
...
func processDeleteMessages(replica ReplicaInterface, msg *msgstream.DeleteMsg, delData *deleteData) error {
	...
	for _, partitionID := range partitionIDs {
		segmentIDs, err := replica.getSegmentIDs(partitionID, segmentTypeGrowing)
		if err != nil {
			return err
		}
	...
	for _, segmentID := range resultSegmentIDs {
		segment, err := replica.getSegmentByID(segmentID, segmentTypeGrowing)
		if err != nil {
			return err
		}
