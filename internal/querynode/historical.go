...
func (h *historical) search(searchMsgID UniqueID, searchReqs []*searchRequest, collID UniqueID, partIDs []UniqueID, plan *SearchPlan,
	...
	for _, partID := range searchPartIDs {
		...
		for _, segID := range segIDs {
			...
			go func() {
				...
				searchResult, err := seg.search(searchMsgID, plan, searchReqs, []Timestamp{searchTs})
				if err != nil {
					err2 = err
					return
				}
