...
func (st *searchMsg) Mergable() bool {
	return true
}
...
func (st *retrieveMsg) Mergable() bool {
	return false
}
...
func canMerge(msg1 *searchMsg, msg2 *searchMsg) bool {
	if msg1.DbID != msg2.DbID {
		return false
	}

	if msg1.CollectionID != msg2.CollectionID {
		return false
	}

	if msg1.DslType != msg2.DslType {
		return false
	}

	if msg1.MetricType != msg2.MetricType {
		return false
	}

	if !funcutil.SortedSliceEqual(msg1.PartitionIDs, msg2.PartitionIDs) {
		return false
	}

	if !bytes.Equal(msg1.SerializedExprPlan, msg2.SerializedExprPlan) {
		return false
	}
