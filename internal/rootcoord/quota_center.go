...
func (q *QuotaCenter) forceDenyReading() {
	...
	// TODO: log.Info("", zap.String("triggerReason", ""))
}
...
func (q *QuotaCenter) calculateRates() error {
	...
	if Params.QuotaConfig.ForceDenyWriting {
		...
		ttFactor, err := q.timeTickDelayed()
		if err != nil {
			return err
		}
...
// timeTickDelayed gets time tick delay of DataNodes and QueryNodes,
// and return the factor according to max tolerable time tick delay.
func (q *QuotaCenter) timeTickDelayed() (float64, error) {
...
func (q *QuotaCenter) checkQueryQueue() float64 {
	...
	for _, metric := range q.queryNodeMetrics {
		...
		if searchNQSum >= maxNQInQueue {
			...
			p := float64(searchNQSum) / float64(maxNQInQueue)
			if p < factor {
				factor = p
			}
			p = float64(queryTasksSum) / float64(maxQueriesInQueue)
			if p < factor {
				factor = p
			}
		}
