...
func (impl *timeTickAppendInterceptor) DoAppend(ctx context.Context, msg message.MutableMessage, append interceptors.Append) (message.MessageID, error) {
	...
	if msg.MessageType() != message.MessageTypeTimeTick {
		...
		defer func() {
			...
			// notify the inspector to perform a sync operation.
			resource.Resource().TimeTickInspector().TriggerSync(impl.operator.Channel())
		}()
...
// GetTimeTickNotifier returns the time tick notifier of interceptor.
func GetTimeTickNotifier(i interceptors.Interceptor) *inspector.TimeTickNotifier {
	if tti, ok := i.(*timeTickAppendInterceptor); ok {
		return tti.operator.TimeTickNotifier()
	}
	return nil
}
